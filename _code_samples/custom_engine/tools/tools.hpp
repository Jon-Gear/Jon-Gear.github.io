...
// Accepts degrees
glm::vec3 RandomDirectionInCone(const glm::vec3& coneDirection, float angle);
glm::vec3 RandomPointOnCircle(float radius);

glm::vec4 lerp(const glm::vec4& start, const glm::vec4& end, const float t);
}  // namespace bee
